...
INSERT INTO vw_updatable_products 
(product_id, product_name, unit_price, units_in_stock, discontinued)
	VALUES(80, 'SimpleTrainingProduct',60,100,0);
...
CREATE VIEW vw_employee_territory_info AS
SELECT 
    e.employee_id,
...
WITH RECURSIVE cte_employeehierarchy AS (
    --Base case: employee with no manager
    SELECT 
        employee_id,
        first_name,
	    last_name,
    ...
    SELECT 
       ...
       e.reports_to,
        cte.level + 1
    FROM employees e
    JOIN cte_employeehierarchy cte ON cte.employee_id = e.reports_to 
)
SELECT level,
	employee_id,
