...
CREATE OR REPLACE FUNCTION enter_product_price()
RETURNS TRIGGER AS $$
BEGIN
...
select *from product_price_audit where product_id=4


--2.Create stored procedure  using IN and INOUT parameters to assign tasks to employees*/
CREATE OR REPLACE PROCEDURE assign_task(IN p_employee_id INT,IN p_task_name VARCHAR(50),INOUT p_task_count INT DEFAULT 0)
LANGUAGE plpgsql
AS $$
BEGIN
